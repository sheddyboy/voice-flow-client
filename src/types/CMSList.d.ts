export interface CMSList {
  ...
  on(
    events: "renderitems" | "additems",
    callback: (result: CMSItem[]) => void
  ): void;
  on(event: "switchpage", callback: (result: number) => void): void;
}
...
interface CMSItemProps {
  [key: string]: {
    ...
    highlightData?: Map<
      string,
      { filterValue?: string; highlightCSSClass: string }
    >;
